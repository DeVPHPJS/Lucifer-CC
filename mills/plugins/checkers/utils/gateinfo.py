        ...
        async def wrap(*args, **kwargs):
            ...
            if user['type'] == "P":
                if user['expire_days'] < time.time():
                    await m.sod("Your Premium Plan has expired. Please contact @DEVPHPJS for renewing your plan.", time = 5)
                ...
                if m.is_private:
                    await m.sod("you dont have to use this command in private chat. talk with @DEVPHPJS for access.", time = 5)
                    return
                ...
                if not m.is_private and not all_chats and m.sender_id not in ADMINS:
                    await m.sod("This chat is unauthorized to use this bot. Please contact @DEVPHPJS for more info.", time = 5)
                    return
            if is_shopify:
                if 'shopify_apis' in user:
                    if len(user['shopify_apis']) < 1:
                        return await m.sod("You have 0 Shopify APIs. Please contact @DEVPHPJS for more info.", time = 5)
